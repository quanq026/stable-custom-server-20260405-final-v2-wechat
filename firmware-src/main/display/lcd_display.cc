...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    lv_coord_t max_width = LV_HOR_RES * 70 / 100 - 16;  // tighter in landscape for readability
...
void LcdDisplay::ScrollChatHistoryBy(int delta_y) {
    DisplayLockGuard lock(this);
    if (content_ == nullptr || delta_y == 0) {
        return;
    }

    // Invert the drag delta so the conversation follows the finger.
    lv_obj_scroll_by(content_, 0, -delta_y, LV_ANIM_OFF);
}
