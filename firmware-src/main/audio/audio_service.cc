...
bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    ...
    if (audio_decode_queue_.size() >= MAX_DECODE_PACKETS_IN_QUEUE) {
        if (wait) {
            ...
            ESP_LOGW(
                TAG,
                "Decode queue full, dropping packet payload=%u queue=%u/%u",
                packet ? static_cast<unsigned>(packet->payload.size()) : 0U,
                static_cast<unsigned>(audio_decode_queue_.size()),
                static_cast<unsigned>(MAX_DECODE_PACKETS_IN_QUEUE)
            );
            return false;
        }
    }
    audio_decode_queue_.push_back(std::move(packet));
    auto queue_size = audio_decode_queue_.size();
    if (queue_size <= 3 || queue_size == MAX_DECODE_PACKETS_IN_QUEUE || queue_size % 10 == 0) {
        ESP_LOGI(
            TAG,
            "Decode queue size now %u/%u",
